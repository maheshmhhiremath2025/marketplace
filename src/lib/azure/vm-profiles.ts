...
export function getProfileForCourse(tags: string[], title: string): LabProfile {
    // Check tags for specific profiles
    if (tags.some(t => t.toLowerCase().includes('docker'))) return LAB_PROFILES['docker'];
    if (tags.some(t => t.toLowerCase().includes('kubernetes') || t.toLowerCase().includes('k8s'))) return LAB_PROFILES['kubernetes'];

    // Check title
    const lowerTitle = title.toLowerCase();
    if (lowerTitle.includes('docker')) return LAB_PROFILES['docker'];
    if (lowerTitle.includes('kubernetes')) return LAB_PROFILES['kubernetes'];
